...
void PutCharHorizontal( uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor, uint16_t bkColor )
{
    ...
    for( i=0; i<16; i++ )
    {
        ...
        for( j=0; j<8; j++ )
        {
            if( ((tmp_char >> (7 - j)) & 0x01) == 0x01 )
            {
                LCD_SetPoint( Ypos + i, Xpos + j, charColor );  /* ???? */
            }
            else
            {
                LCD_SetPoint( Ypos + i, Xpos + j, bkColor );  /* ???? */
            }
        }
    }
}
...
void LCD_Empty_Rectangle( uint16_t x0, uint16_t y0, uint16_t larghezza, uint16_t altezza ,uint16_t spessore, uint16_t colore ){
	LCD_DrawRectangle(x0,y0,spessore,altezza,colore);
	LCD_DrawRectangle(x0,y0,larghezza,spessore,colore);
	LCD_DrawRectangle(x0,y0+altezza-spessore,larghezza,spessore,colore);
	LCD_DrawRectangle(x0+larghezza-spessore,y0,spessore,altezza,colore);
}
